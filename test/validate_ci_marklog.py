...
def unique_mark_entries(values: list[str]) -> list[str]:
    return list(dict.fromkeys(values))


def expected_write_entries(expected_locations: list[str]) -> list[str]:
    # In write mode, some reverse-sync updates are logged without duration.
    write_only_durationless = [
        "Plex/JellyPlex-CI/jellyplex_watched/TV Shows/Breaking Bad/Cat's in the Bag...",
        "Plex/JellyPlex-CI/jellyplex_watched/TV Shows/Doctor Who (2005)/The End of the World",
        "Plex/JellyPlex-CI/jellyplex_watched/TV Shows/Monarch: Legacy of Monsters/Departure",
        "Jellyfin/Jellyfin-Server/JellyUser/Shows/Breaking Bad/Pilot",
        "Emby/Emby-Server/jellyplex_watched/TV Shows/Breaking Bad/Pilot",
    ]

    # This entry is not produced in the write-run flow.
    write_only_exclusions = {
        "Emby/Emby-Server/jellyplex_watched/TV Shows/Monarch: Legacy of Monsters/The Way Out"
    }

    return [
        entry
        for entry in unique_mark_entries(expected_locations + write_only_durationless)
        if entry not in write_only_exclusions
    ]
...
def main():
    ...
    # mark.log entries are unique, so de-duplicate expected combined outputs first.
    expected_locations = unique_mark_entries(
        expected_emby + expected_plex + expected_jellyfin
    )
    ...
    expected_write = expected_write_entries(expected_locations)
